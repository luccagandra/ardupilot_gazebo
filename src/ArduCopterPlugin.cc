  ...
  const bool &_verbose = false)
{
  ...
  if (_verbose) {
    gzerr << "[ArduCopterPlugin] Please specify a value for parameter [" << _name
          << "].\n";
  }
  return false;
}
...
public:
  static constexpr double kDefaultFrequencyCutoff = 5.0;

public:
  static constexpr double kDefaultSamplingRate = 0.2;
};
...
public:
  ArduCopterSocketPrivate socket_in;

  /// \brief Ardupilot Socket to send state to Ardupilot
public:
  ArduCopterSocketPrivate socket_out;
...
/////////////////////////////////////////////////
ArduCopterPlugin::ArduCopterPlugin() : dataPtr(new ArduCopterPluginPrivate)
{
...
void ArduCopterPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (_sdf->HasElement("rotor")) {
    ...
    while (rotorSDF) {
      ...
      if (rotorSDF->HasAttribute("id")) {
        ...
        gzwarn << "id attribute not specified, use order parsed [" << rotor.id << "].\n";
      }
      ...
      if (rotor.joint == nullptr) {
        gzerr << "Couldn't find specified joint [" << rotor.jointName
      ...
      getSdfParam<double>(
        rotorSDF, "rotorVelocitySlowdownSim", rotor.rotorVelocitySlowdownSim, 1);
      ...
      getSdfParam<double>(
        rotorSDF, "frequencyCutoff", rotor.frequencyCutoff, rotor.frequencyCutoff);
      getSdfParam<double>(
        rotorSDF, "samplingRate", rotor.samplingRate, rotor.samplingRate);
  ...
  if (!this->dataPtr->imuSensor) {
    ...
    if (!this->dataPtr->imuSensor) {
      gzerr << "imu_sensor [" << imuName << "] not found, abort ArduPilot plugin.\n"
            << "\n";
      return;
    }
  }
  ...
  if (!InitArduCopterSockets(_sdf)) { return; }
  ...
  this->dataPtr->updateConnection =
    event::Events::ConnectWorldUpdateBegin(std::bind(&ArduCopterPlugin::OnUpdate, this));
...
void ArduCopterPlugin::ApplyMotorForces(const double _dt)
{
  ...
  for (size_t i = 0; i < this->dataPtr->rotors.size(); ++i) {
    double velTarget = this->dataPtr->rotors[i].multiplier * this->dataPtr->rotors[i].cmd
...
void ArduCopterPlugin::ReceiveMotorCommand()
{
  ...
  while (true) {
    // last_pkt = pkt;
    recvSize_last = this->dataPtr->socket_in.Recv(&last_pkt, sizeof(ServoPacket), 0ul);
    if (recvSize_last == -1) { break; }
    counter++;
    pkt = last_pkt;
  }
  if (counter > 0) { gzdbg << "Drained n packets: " << counter << std::endl; }
    ...
    for (unsigned i = 0; i < this->dataPtr->rotors.size(); ++i) {
      if (i < MAX_MOTORS) {
        ...
        this->dataPtr->rotors[i].cmd =
          this->dataPtr->rotors[i].maxRpm * pkt.motorSpeed[i];
      } else {
        gzerr << "too many motors, skipping [" << i << " > " << MAX_MOTORS << "].\n";
      }
    }
  }
}
...
void ArduCopterPlugin::SendState() const
{
  ...
  ignition::math::Vector3d linearAccel = this->dataPtr->imuSensor->LinearAcceleration();
  ...
  ignition::math::Vector3d angularVel = this->dataPtr->imuSensor->AngularVelocity();
  ...
  ignition::math::Pose3d worldToModel = gazeboToNED + this->dataPtr->model->WorldPose();
