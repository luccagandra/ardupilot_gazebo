...
public:
  std::unique_ptr<rendering::SelectionBuffer> selectionBuffer;
...
ArduCopterIRLockPlugin::ArduCopterIRLockPlugin()
  : SensorPlugin(), dataPtr(new ArduCopterIRLockPluginPrivate)
{
  // socket
...
#endif
  int one = 1;
  setsockopt(this->dataPtr->handle,
    IPPROTO_TCP,
    TCP_NODELAY,
...
void ArduCopterIRLockPlugin::Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
  ...
  this->dataPtr->connections.push_back(
    this->dataPtr->parentSensor->Camera()->ConnectNewImageFrame(
      std::bind(&ArduCopterIRLockPlugin::OnNewFrame,
        this,
        std::placeholders::_1,
        std::placeholders::_2,
        std::placeholders::_3,
  ...
  const std::string & /*_format*/)
{
  ...
  if (!this->dataPtr->selectionBuffer) {
    ...
    this->dataPtr->selectionBuffer.reset(new rendering::SelectionBuffer(cameraName,
  ...
  for (const auto &f : this->dataPtr->fiducials) {
    ...
    if (!camera->IsVisible(vis)) continue;
    ...
    if (entity && !entity->getUserObjectBindings().getUserAny().isEmpty()) {
      try {
        result = scene->GetVisual(
          Ogre::any_cast<std::string>(entity->getUserObjectBindings().getUserAny()));
  ...
  unsigned int _y)
{
  ...
  const float angleX =
    static_cast<float>((static_cast<double>(_x) - (imageWidth * 0.5)) / pixelsPerRadianX);
